...
int main()
{
	SFMLWindowSettings window_settings("Space Invader Labyrinth");
	window_settings.videoMode = sf::VideoMode::getDesktopMode();
	window_settings.setWidthAndHeight = false;
	window_settings.windowFlags = sf::Style::Fullscreen;

	IInput::SPtr input = std::make_shared<DE::SFMLInput>();
	IWindow::SPtr window = std::make_shared<DE::SFMLWindow>(input, window_settings);
	...
	ISimulation::SPtr simulation = std::make_shared<MazeLevel>();
